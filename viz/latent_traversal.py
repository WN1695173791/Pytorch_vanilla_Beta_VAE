...
class LatentTraverser:
    def __init__(self, latent_spec):
        ...
        if self.is_both_continue:
            self.cont_dim = latent_spec['cont_var'] + latent_spec['cont_class']
    ...
    def traverse_line(self, cont_idx=None, disc_idx=None, size=5, both_continue=False, both_discrete=False):
        """
    ...
    def traverse_grid(self, cont_idx=None, cont_axis=None, disc_idx=None,
                      disc_axis=None, size=(5, 5), batch_chairs=None, real_im=False, model=None, nb_samples=1, both_continue=False, both_discrete=False):
        """
        ...
        if real_im:
            ...
            x_recon, latent_dist, latent_sample, latent_sample_cont, latent_sample_disc, prediction, \
            prediction_random_continue, prediction_random_discret, prediction_zc_pert_zd, prediction_zc_zd_pert = model(input_data, both_continue=both_continue)
            model.train()
            img_latent_cont = latent_sample[:nb_samples, :self.cont_dim]

        if both_continue:
            samples.append(self._traverse_continuous_grid(idx=cont_idx,
                                                          axis=cont_axis,
                                                          size=size,
                                                          img_latent=img_latent_cont))
        elif both_discrete:
            for i, disc_dim in enumerate(self.disc_dims_var):
                if i == disc_idx:
    ...
    def _traverse_continuous_grid(self, idx, axis, size, img_latent=None):
        """
        ...
        if img_latent is not None:
            samples = np.repeat(img_latent.detach().numpy(), num_samples, axis=0).reshape(
                (num_samples, self.cont_dim))
        elif self.sample_prior:
