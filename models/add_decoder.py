...
class Add_decoder(nn.Module, ABC):
    ...
    def __init__(self,
        ...
        """
        super(Add_decoder, self).__init__()
        ...
        if self.three_conv_layer:
            ...
            self.decoder += [
                nn.ConvTranspose2d(self.hidden_filters_3, self.hidden_filters_2, self.kernel_size_3, stride=self.stride_size),
                nn.ReLU(True),
                # PrintLayer(),
            ]
        if self.two_conv_layer:
            ...
            self.decoder += [
                nn.ConvTranspose2d(self.hidden_filters_2, self.hidden_filters_1, self.kernel_size_2, stride=self.stride_size),
                nn.ReLU(True),
                # PrintLayer(),
            ]
        self.decoder += [
    ...
    def forward(self, x):
        ...
        z_struct = self.pre_trained_model(x)
