...
class Custom_CNN_BK(nn.Module, ABC):
    ...
    def __init__(self,
        ...
        if self.two_conv_layer:
            self.model += [
                nn.Conv2d(self.nc, self.hidden_filters_1, self.kernel_size_2, stride=self.stride_size),
                nn.BatchNorm2d(self.hidden_filters_1),
                nn.ReLU(True),
                PrintLayer(),  # B, 32, 25, 25
            ]
        if self.three_conv_layer:
            self.model += [
                nn.Conv2d(self.nc, self.hidden_filters_2, self.kernel_size_3, stride=self.stride_size),
                nn.BatchNorm2d(self.hidden_filters_1),
                nn.ReLU(True),
                PrintLayer(),  # B, 32, 25, 25
            ]
        ...
        self.model += [
            ...
            View((-1, self.hidden_filters_1)),  # B, hidden_filters_1
            PrintLayer(),  # B, hidden_filters_1
            nn.Linear(self.hidden_filters_1, self.z_struct_size),  # shape: (Batch, z_struct_size)
