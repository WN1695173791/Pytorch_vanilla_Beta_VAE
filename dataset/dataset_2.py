# https://github.com/YannDubs/disentangling-vae/blob/master/utils/datasets.py
...
def get_dataloaders(dataset, root=None, shuffle=True, pin_memory=True,
                    batch_size=64, logger=logging.getLogger(__name__), **kwargs):
    ...
    percent_train = 0.8
    percent_valid = 0.1
    # nb test example is the rest
    n_train_examples = int(len(dataset) * percent_train)
    n_valid_examples = int(len(dataset) * percent_valid)
    n_test_examples = len(dataset) - n_train_examples - n_valid_examples
    dataset_train, dataset_valid, dataset_test = torch.utils.data.random_split(dataset, [n_train_examples,
                                                                                         n_valid_examples,
                                                                                         n_test_examples])

    return DataLoader(dataset_train, batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, **kwargs), \
           DataLoader(dataset_valid, batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, **kwargs), \
           DataLoader(dataset_test, batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, **kwargs)
